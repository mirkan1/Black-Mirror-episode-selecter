...
def last_episode():
    ...
    try:
        episode_show.config(
            text="The episode you\rRandomed last time is\r{}".format(split_read_txt[-1]))
        print(split_read_txt)
    except:
        episode_show.config(
            text="Didnt watch any episode yet\nClick the Button")
        print(split_read_txt)
...
episode_demand = Button(mainframe, command=episode_selecter, width=22,
                        text='Click for random episode', fg="black", bg="white", font="Arial 16 bold")
episode_demand.grid(column=0, row=0)

episode_show = Label(mainframe, text="as", height=6, width=22,
